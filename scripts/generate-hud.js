...
async function main() {
  ...
  const user = await fetchJson('https://api.github.com/users/PedroReoli', token) || { public_repos: 45 };
  const repos = await fetchJson('https://api.github.com/users/PedroReoli/repos?per_page=100&sort=updated', token) || [];

  let lastActiveRepo = { name: 'Organon', updated_at: new Date().toISOString(), language: 'TypeScript' };
  
  if (Array.isArray(repos) && repos.length > 0) {
    const nonFork = repos.filter(r => !r.fork);
    if (nonFork.length > 0) {
      lastActiveRepo = nonFork[0];
    }
  }
  ...
  const totalPublic = Array.isArray(repos) ? repos.filter(r => !r.fork).length : 45;

  const svgContent = `<svg fill="none" width="850" height="340" viewBox="0 0 850 340" xmlns="http://www.w3.org/2000/svg">
  <foreignObject width="100%" height="100%">
    <div xmlns="http://www.w3.org/1999/xhtml">
      <style>
        ...
        @keyframes pulseGlow {
          0%, 100% { opacity: 0.9; filter: drop-shadow(0 0 8px #38bdf8); }
          50% { opacity: 0.4; filter: drop-shadow(0 0 2px #38bdf8); }
        }
        @keyframes scanline {
          0% { top: 0%; }
          100% { top: 100%; }
        ...
        .hud-wrapper {
          ...
          position: relative;
          overflow: hidden;
          animation: fadeInHUD 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
      ...
      <div class="hud-wrapper">
        ...
        <div class="hud-header">
          <div class="brand">
            ...
            <div class="title">REOLI OS // CYBER HUD v2.4</div>
          </div>
          <div class="live-tag">
        ...
        <div class="hud-grid">
          <div class="card">
            <div class="card-label">🔥 ÚLTIMO PROJETO ATIVO</div>
            <div class="card-val" style="font-size: 16px;">${lastActiveRepo.name}</div>
            <div class="card-sub">Stack: ${lastActiveRepo.language || 'TypeScript'}</div>
          </div>

          <div class="card">
            <div class="card-label">⚡ REPOSITÓRIOS PÚBLICOS</div>
            <div class="card-val">${totalPublic}</div>
            <div class="card-sub">Ativos &amp; Open Source</div>
          </div>

          <div class="card">
            <div class="card-label">🧠 ARQUITETURA &amp; IA</div>
            <div class="card-val" style="font-size: 15px;">Spec-Driven Engine</div>
            <div class="card-sub">Agents &amp; Full Stack</div>
          </div>
        </div>

        <div class="footer-bar">
          <span>HOST: PEDRO REOLI</span>
          <span>ÚLTIMA ATUALIZAÇÃO: <span class="timestamp">${formattedDate}</span></span>
  ...
  console.log('Advanced 3D Cyber HUD SVG generated successfully!');
}
